...
export function LevelMeter({
  ...
  return (
    <div className="space-y-1">
      <div className="relative h-3.5 w-full overflow-hidden rounded-full border-[1.5px] border-ink bg-desk">
        <div ref={barRef} className="h-full bg-mint transition-[width] duration-75" style={{ width: "0%" }} />
        <div
          className="absolute top-0 h-full w-[2px] bg-maple"
          style={{ left: `${Math.min(100, (threshold / 0.5) * 100)}%` }}
          title="Noise gate"
        />
      </div>
      <div className="flex items-center gap-2 hint">
        <div ref={dotRef} className="h-2.5 w-2.5 shrink-0 rounded-full bg-maple" style={{ opacity: 0.25 }} />
        <span>Green is your voice. The mouth only moves past the red line.</span>
      </div>
    </div>
  );
}
